...
import { ArrowRight, CalendarDays, CheckCircle2, FileText, LayoutDashboard, Mail, MessageSquareText } from "lucide-react";
import { buildGmailConnectUrl, fetchGmailConnectionStatus, getStoredAuthUser } from "../../../lib/supabaseAuth";
...
export default function DashboardHome() {
    ...
    useEffect(() => {
        ...
        fetchGmailConnectionStatus(user.email)
            .then(setGmailStatus)
            .catch((err) => setError(err.message || "Unable to load Gmail status."));
    }, [user?.email]);
    ...
    const connectGmail = async () => {
        ...
        try {
            ...
            setError(err.message || "Unable to start Gmail connect.");
            setLoading(false);
        }
    };

    const features = [
        { icon: FileText, title: "Documents", desc: "Browse, upload, and manage your document library with ease.", href: "/documents" },
        { icon: MessageSquareText, title: "Chat with PDF", desc: "Select PDFs and ask AI-powered questions in real time.", href: "/chat" },
        { icon: CalendarDays, title: "Calendar", desc: "Schedule events with priority highlighting and smart reminders.", href: "/calendar" },
    ];
            ...
            <div className="w-full max-w-5xl grid gap-6">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                {user?.email ? (
                                    <p className="mt-2 text-sm text-[#413632]/60">Signed in as {user.email}</p>
                                ) : null}
                            </div>
                        </div>
                        ...
                        <div className="rounded-2xl border border-[#CABDB2]/70 bg-[#FFFBF0] p-4 min-w-[280px]">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-0">
                                    <p className="text-xs uppercase tracking-[0.2em] text-[#413632]/55">Gmail connection</p>
                                    <p className="font-semibold text-sm">{gmailStatus.connected ? "Connected" : "Not connected"}</p>
                                    <p className="text-xs text-[#413632]/70 truncate">{gmailStatus.connection?.google_email || user?.email || "No inbox linked yet"}</p>
                                </div>
                            </div>
                            <button
                            ...
                            >
                                ...
                                {loading ? "Opening Google..." : gmailStatus.connected ? "Reconnect Gmail" : "Connect Gmail"}
                            </button>
                            {error ? <p className="mt-3 text-xs text-[#8C4F3E]">{error}</p> : null}
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full">
                    {features.map((f) => {
                        const Icon = f.icon;
                        return (
                            <Link key={f.href} href={f.href} className="group flex items-start gap-4 p-5 rounded-2xl border border-[#CABDB2]/60 bg-card hover:border-[#CA8A78] hover:bg-[#FFEAD5]/80 transition-all duration-200">
                                ...
                                <div className="min-w-0">
                                    <h2 className="text-sm font-bold text-[#413632] flex items-center gap-2">
                                        {f.title}
                                        <ArrowRight size={13} className="text-[#CA8A78] opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all" />
                                    </h2>
